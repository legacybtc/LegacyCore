# Legacy Core Documentation Index

Purpose: provide a clean map of public Legacy Core documentation by audience.  
Audience: users, operators, integrators, and developers.  
Status: v1.0.5 documentation index.
Safety warning: keep RPC (`19556`) private unless you have strict network controls.

## For Normal Users

- [Getting Started](GETTING_STARTED.md)
- [Wallet Backup and Restore](WALLET_BACKUP_AND_RESTORE.md)
- [Mining](MINING.md)
- [Troubleshooting](TROUBLESHOOTING.md)

## For Node Operators

- [Running a Node](RUNNING_A_NODE.md)
- [Seed Node Operator Guide](SEED_NODE_OPERATOR.md)
- [Monitoring](MONITORING.md)
- [P2P Protocol Notes](P2P_PROTOCOL.md)

## For Integrations

- [RPC](RPC.md)
- [Pool Integration](POOL_INTEGRATION.md)
- [Exchange Integration](EXCHANGE_INTEGRATION.md)
- [Explorer Integration](EXPLORER_INTEGRATION.md)
- [Confirmations and Reorgs](CONFIRMATIONS_AND_REORGS.md)

## For Developers / Release

- [Storage and Reindex](STORAGE_AND_REINDEX.md)
- [Release Process](RELEASE_PROCESS.md)
- [Release Scorecard](RELEASE_SCORECARD.md)
- [Security Model](SECURITY_MODEL.md)
- [Windows Service Notes](WINDOWS_SERVICE.md)
- [Multi-node Testing](MULTINODE_TESTING.md)
- [Release Notes Template](RELEASE_NOTES_TEMPLATE.md)
