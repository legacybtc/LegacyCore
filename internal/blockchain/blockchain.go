...
func (c *Chain) ProcessBlockWithResult(block *wire.MsgBlock, precomputedHash ...string) (BlockProcessResult, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.processBlockLocked(block, precomputedHash...)
}
...
func (c *Chain) processBlockLocked(block *wire.MsgBlock, precomputedHash ...string) (BlockProcessResult, error) {
	result, err := c.blockProcessPreviewLocked(block, true, precomputedHash...)
	if err != nil || result.Status != "" {
		return result, err
	}
	hashStr := result.Hash
...
func (c *Chain) blockProcessPreviewLocked(block *wire.MsgBlock, mutate bool, precomputedHash ...string) (BlockProcessResult, error) {
	...
	var hashStr string
	if len(precomputedHash) > 0 && precomputedHash[0] != "" {
		hashStr = precomputedHash[0]
	} else {
		hash, err := c.HashHeader(block.Header)
		if err != nil {
			return result, err
...
func (c *Chain) ValidateHeaderSequence(headers []wire.BlockHeader) ([]chainhash.Hash, error) {
	...
	if !startKnown {
		if canonical := c.legacyByHash[startHashString]; canonical != "" {
			startHeight, startKnown = c.activeHeight(canonical)
			if startKnown {
				startHashString = canonical
			}
		}
	}
	...
	for i, header := range headers {
		...
		if header.Bits != expectedBits {
			return nil, fmt.Errorf("header %d has unexpected bits: got %08x, want %08x (genesisBits=%08x postGenesisBits=%08x recent_len=%d startHeight=%d)", i, header.Bits, expectedBits, genesisBits, postGenesisBits, len(recent), startHeight)
		}
