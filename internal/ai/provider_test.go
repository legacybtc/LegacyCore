...
func TestMockProviderChat(t *testing.T) {
	...
	if !strings.Contains(resp.String(), "mining") && !strings.Contains(resp.String(), "Mining") {
		t.Fatalf("no mining in response: %s", resp.String())
	}
}
...
func TestLifecycleManagerChatWhenStopped(t *testing.T) {
	...
	if err == nil {
		t.Fatal("should fail when stopped")
	}
}
